...
#ifdef LATENCY
double T = 0.5;     // total time to take N timesteps
size_t N = 10;      // # of timestep (10 for no latency)
#else
double T = 1;       // total time to take N timesteps
size_t N = 20;      // # of timestep (10 for no latency)
#endif
double dt = T/N;    // time that elpases between actuations (0.1 for no latency)
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    const int weight_cte = 1; 
    const int weight_epsi = 1; 
    ...
    const int weight_delta = 2;
    const int weight_a = 2;
    ...
    const int weight_delta_change = 15000;
    const int weight_a_change = 100;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // save predicted x, y points for the simulator
  for (uint i = 0; i < N-1; i++) {
    ptsx.push_back(solution.x[x_start + i + 1]);
    ptsy.push_back(solution.x[y_start + i + 1]);
  }

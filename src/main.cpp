      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          const int n_next_vals = 30;
          const double step = 2.0;
          double sx;
          for (uint x = 0; x < n_next_vals; x++) {
            sx = x*step;
            next_x_vals.push_back(sx);
            next_y_vals.push_back(polyeval(coeffs, sx));
          }
          ...
          // SUBMITTING.
#ifdef LATENCY
          const int latency_ms = latency*1000;
#else
          const int latency_ms = 100;
#endif
          this_thread::sleep_for(chrono::milliseconds(latency_ms));//100));

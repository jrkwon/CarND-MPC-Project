...
The prediction horizon *T* is the duration over which future predictions are made. *T* is the product of two variables, *N* and *dt*. A good approach to setting *N*, *dt*, and *T* is to first determine a reasonable range for *T* and then tune *dt* and *N* appropriately, keeping the effect of each in mind. I started with 1.0 sec for *T*. The initial combination of *N* was chosen as 10 then *dt* is automatically defined as 0.01. It worked OK until the track meets continuous sharp turns. I increased the timestep to 20, then again *dt* is automatically selected as 0.05. This time the driving is a little bit smoother and manages to drive the whole track. See the [demo](https://youtu.be/3n9QVA0b-RE).
